...
st.title("Detect and Recognize Single Image")

st.header("Chose your image from PC")
...
if img is not None:
    ...
    # To See details
    imgp = cv2.imread("test_images/" + img.name)
    imgp, veri = singleImage(imgp, night)
    cv2.imwrite("output_testImage/" + img.name, imgp)
    st.subheader("Analysing Image")
    st.image("output_testImage/" + img.name)
    os.remove("output_testImage/" + img.name)
